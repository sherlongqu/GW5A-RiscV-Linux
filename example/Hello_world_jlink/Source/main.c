...
uint8_t uart_receive(){
    uint32_t uart_base = 0x80000010;
    volatile uint8_t* uart_rx = (volatile uint8_t*) uart_base + 0x02;
    return *uart_rx;
}
...
int main(void) {
    ...
    for (i = 0; 1; i++) {
        ...
        if (uart_get_sta()) {
            received_byte = uart_receive(); // Receive data from UART
            uart_send(received_byte);      // Send received data back to UART
        }

        sprintf((char*)print_buf, "Up Time: %d s\r\n", i);
        uart_send_arr(print_buf, strlen((char*)print_buf));
        sprintf((char*)print_buf, "slgp1\r\n", i);
        uart_send_arr(print_buf, strlen((char*)print_buf));
    }
}
